...
from dcxml.xmlutils import Rules, dump_etree_helper, etree_to_string
...
def rule_factory(plural, singular, attrib=None):
    ...
    def f(path, values):
        for v in values:
            if v:
                ...
                if attrib is not None and isinstance(v, dict):
                    elem.set(attrib, v[attrib])
                    elem.text = v["text"]
                else:
                    elem.text = v
                yield elem
...
creators = rule_factory('creators', 'creator', attrib="id")
